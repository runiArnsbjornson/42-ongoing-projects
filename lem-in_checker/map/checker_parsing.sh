...
intro()
{
	echo "\033[31mTests could not have only one interpretation,\nthink and make your own if you don't agree with one test"
	echo "If you use it to correct someone, be careful and be open minded. \033[0m"
}
...
test_fichier()
{
	...
	then
		...
		echo "\t[ \033[32m OK \033[0m ]\c"
		if [ "$pb" -ne 0 ] && [ $1 -eq 1 ]
		then
			echo " Error exit status : check this test"
		else
			echo ""
		fi
	fi
...
new_test()
{
	print_title "$3"
	map_dir=$1
	echo "\033[35mPATH : " $map_dir "\033[0m"
