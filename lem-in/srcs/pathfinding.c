...
/*   Updated: 2017/06/14 18:09:50 by jdebladi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		hacking_way(t_data *data, int room, int turn, int try)
{
	...
	{
		data->p[data->s[try][turn - 1]][room] = 0;
		data->s[try][turn - 1] = -1;
		return (1);
	}
...
int		get_ways(t_data *data, int room, int turn, int try)
{
	...
	i = -1;
	while (++i < data->rooms)
		ft_printf("[%d]%c", data->mark[i], i == data->rooms - 1 ? '\n': ' ');
...
void	set(t_data *data)
{
	...
	pos.x = -1;
	while (++pos.x < (data->lmax + data->rooms) * 2)
	{
		pos.y = -1;
		while (++pos.y < (data->lmax + 1) * 2)
...
void	pathfinding(t_data *data)
{
	...
	data->len = data->lmax;
	if (!(data->s = ft_inttab((data->lmax + 1) * 2, (data->lmax + data->rooms) * 2)) || !(data->mark = (int *)ft_memalloc(sizeof(int) * (unsigned long)(data->rooms + 1))))
		ft_error(data, "Error malloc");
	set(data);
	try = -1;
	while (++try < (data->lmax + data->rooms) * 2)
	{
		i = -1;
		while (++i < data->rooms)
			data->mark[i] = 0;
